...
class RandomMaze(object):
        ...
        def simply_maze(self, density=5, complexity=5):
            """
            ...
            for i in range(density):
                    ...
                    for j in range(complexity):
                            neighbour = []
                            if x > 1:
                                    neighbour.append((y, x - 1))
                            if x < shape_width - 2:
                                    neighbour.append((y, x + 1))
                            if y > 1:
                                    neighbour.append((y - 1, x))
                            if y < shape_height - 2:
                                    neighbour.append((y + 1, x))
                            if len(neighbour) > 0:
        ...
        def dfs_maze(self):
                ...
                # let the first row and the last row become False 
                maze[0] = [0]*self.width
                ...
                #path_track.append((2, 2))
                #path_track.append((2*cell_row_num, 2*cell_col_num + 1))                 #....remove a path block
